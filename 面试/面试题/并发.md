...
3. 三个特性，volatile和sychronized的区别，volatile的原理，如何实现单例？

   1. JMM 即 Java Memory Model，它从java层面定义了主存、工作内存抽象概念，底层对应着 CPU 寄存器、缓存、硬件内存、CPU 指令优化等：https://www.iteye.com/blog/greemranqq-2010004

   2. java内存模型定义了一些规则来禁止cpu缓存和编译器优化，happen-before用来描述两个操作的内存的可见性，有以下6条：
...
8. AQS
   ...
   1. 原理：AQS  核心思想是，如果被请求的共享资源空闲，则将当前请求资源的线程设置为有效的工作线程，并且将共享资源设置为锁定状态。如果被请求的共享资源被占用，那么就需要一套线程阻塞等待以及被唤醒时锁分配的机制，这个机制 AQS 是用 CLH(Craig,Landin,and Hagersten队列是一个虚拟的双向队列) 队列锁实现的，即将暂时获取不到锁的线程加入到队列中
...
13. [什么是协程](https://blog.csdn.net/zheng199172/article/details/88800275)
